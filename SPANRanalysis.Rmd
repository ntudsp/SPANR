...
Degree of agreement using Bland-Altman statistics is detailed in section [Degree of agreement](#agreement)

### ART Anova Annoyance

This code block generates the ART anova statistics for perceived annoyance (PAY), as shown in Table C.2.
...
The circumplexity analysis is detailed in the section: [circumplexity analysis](#circumplex)
...
The following code block generates the 2D density plots of the ISOPL and ISOEV scores, as shown in Figure 3.
...
This code block generates the Pearson's correlation across all objective and subjective indices as shown in Table C.7. 
...
m.annoy.obj<-lm(annoyscale~`Max(C)`+`L10(A)`+S,
                data=comb.obj.subj.mean)
summary(m.annoy.obj)
...
m.loud.air<-lm(mean_loud~`L(C)`,data=cbind(dv.unscaled.loud.air,
                                 pred.unscaled.loud.air))
summary(m.loud.air)

ggplot(cbind(dv.unscaled.loud.air,pred.unscaled.loud.air), 
       aes(x = `L(C)`, y = mean_loud)) + 
  geom_point() +
...
m.loud.mrt<-lm(mean_loud~`L95(C)`+`L50(A)`,data=cbind(dv.unscaled.loud.mrt,
                                 pred.unscaled.loud.mrt))
summary(m.loud.mrt)
...
m.loud.tra<-lm(mean_loud~`N95`,data=cbind(dv.unscaled.loud.tra,
                                 pred.unscaled.loud.tra))
summary(m.loud.tra)

ggplot(cbind(dv.unscaled.loud.tra,pred.unscaled.loud.tra), 
       aes(x = `N95`, y = mean_loud)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")
...
This code block generates the summary statistics and two-way ART ANOVA statistics for the sentiment analysis, as shown in Table C.5 and C.6, respectively.
...
The following code block generates the Bland-Altman statistics and plots for the perceived annoyance (PAY), as shown in Figure B.1. 
...
The following code blocks generate the statistics as shown in Appendix C in the paper.

### Summary stats

This code block computes the summary statistics of the quantitative subjective evaluation scores, as shown in Table C.1.
...
### Circumplexity analysis {#circumplex}
